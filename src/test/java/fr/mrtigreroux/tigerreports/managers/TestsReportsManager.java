...
public class TestsReportsManager {
    ...
    public static final Map<ReportsManager, Integer> waitingUpdateDataEndTaskIdByReportsManager =
            new HashMap<>();
    
    public static void setupReportsManagerForAddListenerWithoutUpdateDataAndMenuUpdater(
            ReportsManager rm, MockedStatic<MenuUpdater> menuUpdaterStaticMock,
            Holder<Integer> menuUpdaterStartIfNeededCalledTimes) {
        doNothing().when(rm)
                .updateDataWhenPossible(any(Database.class), any(TaskScheduler.class), any(UsersManager.class));
        menuUpdaterStaticMock
                .when(() -> MenuUpdater.startIfNeeded(any(ReportsManager.class), any(Database.class), any(TaskScheduler.class), any(UsersManager.class))).thenAnswer((Answer<Void>) answer -> {
                    if (menuUpdaterStartIfNeededCalledTimes != null) {
                        menuUpdaterStartIfNeededCalledTimes
                                .set(menuUpdaterStartIfNeededCalledTimes.get() + 1);
                    }
                    return null;
                });
    }
    ...
    public static class FakeReportsPageListener {
        ...
        public final ReportsPage.ReportsPageListener reportsPageListener =
    ...
    public static void whenReportsManagerIsNotUpdatingData(ReportsManager rm, TaskScheduler ts,
            long checkInterval, Runnable run) {
        if (!rm.isPendingDataUpdate()) {
            LOGGER.debug(
                    () -> "whenReportsManagerIsNotUpdatingData(): rm is not pending data update, no periodic check needed"
            );
            run.run();
            return;
        }
        LOGGER.debug(
                () -> "whenReportsManagerIsNotUpdatingData(): rm is pending data update, will periodically check it..."
        );
        waitingUpdateDataEndTaskIdByReportsManager
                .put(rm, ts.runTaskRepeatedly(checkInterval, checkInterval, () -> {
                    if (!rm.isPendingDataUpdate()) {
                        LOGGER.debug(
                                () -> "whenReportsManagerIsNotUpdatingData(): rm is no longer pending data update"
                        );
                        ...
                        LOGGER.debug(
                                () -> "whenReportsManagerIsNotUpdatingData(): rm is still pending data update..."
                        );
                    }
                }));
    }

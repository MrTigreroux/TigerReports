...
public class RandomUtils {
    ...
    public static <T> int executeActionOnRandomElements(List<T> elements, int minPickedAmount,
            int maxPickedAmount, Consumer<T> pickedElementAction) {
        ...
        for (int i = 0; i < elements.size(); i++) {
            ...
            if (
                rand.nextBoolean()
                        || ((elements.size() - i) <= (minPickedAmount - pickedElementsAmount))
            ) { // testsReportsData.size() - i = amount of remaining reports counting the current i
                int fi = i;
                int fpickedReportsAmount = pickedElementsAmount;
                LOGGER.debug(
                        () -> "executeActionOnRandomElements(): picked element at i = " + fi + ", "
                                + (elements.size() - fi) + ", "
                                + (minPickedAmount - fpickedReportsAmount)
                );
        ...
        assertTrue(
                pickedElementsAmount >= minPickedAmount,
                () -> "pickedElementsAmount = " + fpickedElementsAmount + ", maxPickedAmount = "
                        + maxPickedAmount
        );
        assertTrue(
                pickedElementsAmount <= maxPickedAmount,
                () -> "pickedElementsAmount = " + fpickedElementsAmount + ", maxPickedAmount = "
                        + maxPickedAmount
        );

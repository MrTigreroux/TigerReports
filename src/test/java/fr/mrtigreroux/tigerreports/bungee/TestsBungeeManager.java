...
public class TestsBungeeManager {
    ...
    public TestsBungeeManager(TigerReports tr, ReportsManager rm, Database db, VaultManager vm,
            UsersManager um, FakeServer localServer, TestsBungeeNetwork testsBungeeNetwork) {
        bm = mock(
                BungeeManager.class,
        ...
        if (!testsBungeeNetwork.isPresent(localServer)) {
            throw new IllegalArgumentException(
                    "The local server " + localServer
                            + " is not present in the tests bungee network"
            );
        }
        ...
        doAnswer((inv) -> {
            ...
            doAnswer((inv2) -> {
                ...
                return null;
                    ...
                    .onReceive(any(Database.class), any(TaskScheduler.class), any(UsersManager.class), any(ReportsManager.class), any(VaultManager.class), any(BungeeManager.class));
            doAnswer((inv2) -> {
    ...
    private void setupSendPluginMessageMock() {
        doAnswer((invocation) -> {
            ...
            switch (subchannel) {
                case "Forward":
                    ...
                    runAfterFakeCommunicationDelay(
                            () -> fakeSendPluginNotificationToNetworkTarget(
                                    networkTarget,
                                    pluginMessage
                            )
                    );
                ...
                case "PlayerList":
                    ...
                    answer = newFakePlayerListAnswer(
                            serverName,
                ...
                default:
                    throw new IllegalStateException(
                            "Unexpected fakePlayerUsedForBungee.sendPluginMessage() with subchannel = "
                                    + subchannel
                    );
            }
            ...
            return null;
        }).when(fakePlayerUsedForBungee)
                .sendPluginMessage(any(Plugin.class), any(String.class), any(byte[].class));
    }
    ...
    public void fakeSendPluginNotificationToNetworkTarget(String networkTarget, byte[] message) {
        if ("ALL".equals(networkTarget)) {
            ...
            if (targetServer == null) {
                throw new IllegalArgumentException(
                        "The network target " + networkTarget + " is unknown"
                );
            }
    ...
    public void fakeSendPluginNotificationToServer(TestsBungeeNetwork.FakeServer server,
            byte[] message) {
        server.getTestsTaskScheduler().runTask(() -> {
            instanceLogger
                    .debug(() -> "fakeSendPluginNotificationToServer(): target = " + server.name);
    ...
    public void executeTaskSendingPluginMessage(Runnable task) {
        try (
            ...
            userUtilsMock.when(() -> UserUtils.getRandomPlayer())
                    .then((invocation) -> fakePlayerUsedForBungee);
    ...
    public void receivePluginMessage(String channel, byte[] message) {
        try (
                MockedStatic<UserUtils> userUtilsMock =
                        mockStatic(UserUtils.class, Mockito.CALLS_REAL_METHODS);
                MockedStatic<BungeeNotificationType> bungeeNotificationTypeMock =
                        mockStatic(BungeeNotificationType.class, Mockito.CALLS_REAL_METHODS)
        ) {
            userUtilsMock.when(() -> UserUtils.getRandomPlayer())
                    .then((inv) -> fakePlayerUsedForBungee);
            bungeeNotificationTypeMock.when(() -> BungeeNotificationType.getById(any(byte.class)))
                    .then((inv) -> {
                        ...
                        if (notifTypeMock != null) {
                            ...
                            throw new IllegalStateException(
                                    "BungeeNotificationType.getById(" + id + ") is not expected"
                            );
                        }
                    });
    ...
    public static <N extends BungeeNotification> N assertCalledOnceSendPluginNotificationToAll(
            BungeeManager bm, Class<N> notifClass) {
        ArgumentCaptor<BungeeNotification> bungeeNotifCaptor =
                ArgumentCaptor.forClass(BungeeNotification.class);
        ...
        throw new IllegalStateException(
                "sendPluginNotificationToAll was not called with " + notifClass.getName()
        );
    }
    
    public static <N extends BungeeNotification> void assertNeverCalledSendPluginNotificationToAll(
            BungeeManager bm, Class<N> notifClass) {

...
public class BungeeManagerTest extends TestClass {
	...
	class SendAndReceiveBungeeNotification {
		...
		@Test
		void test1() {
			testSendAndReceiveBungeeNotification(new NewReportBungeeNotification(100L, "reportServer", false, "reportBasicData"));
		}

		@Test
		void test2() {
			testSendAndReceiveBungeeNotification(new StatusBungeeNotification(101L, 5, "statusDetails"));
		}
		...
		void testSendAndReceiveBungeeNotification(BungeeNotification notif) {
			...
			assertTrue(network.server1.getTestsTaskScheduler().runTaskAndWait((tc) -> {
				network.server2.getTestsTaskScheduler().runTask(() -> {
					try {
						network.server2.getTestsBm().expectPluginNotification(BungeeNotificationType.getByDataClass(PlayerOnlineBungeeNotification.class), (receivedNotif, normalExec) -> {
							// Do normal behavior
						...
						network.server2.getTestsBm().expectPluginNotification(notifType, (receivedNotif, normalExec) -> {
					...
					network.server1.getTestsTaskScheduler().runTask(() -> {
						try {
							network.server1.getTestsBm().expectPluginNotification(BungeeNotificationType.getByDataClass(PlayerOnlineBungeeNotification.class), (receivedNotif, normalExec) -> {
								// Do normal behavior
								try (MockedStatic<Bukkit> bukkitMock = mockStatic(Bukkit.class, Mockito.CALLS_REAL_METHODS)) {
									bukkitMock.when(() -> Bukkit.getPlayer(anyString())).thenReturn(null);
									normalExec.run();
						...
						network.server1.getTestsBm().executeTaskSendingPluginMessage(() -> {
							network.server1.playerConnection(FakeServer.newFakeOnlinePlayer("player1"));
							network.server2.getTestsTaskScheduler().runTask(() -> {
								network.server2.getTestsBm().executeTaskSendingPluginMessage(() -> {
									network.server2.playerConnection(FakeServer.newFakeOnlinePlayer("player2"));
			
									network.server1.getTestsTaskScheduler().runTask(() -> {
										network.server1.getTestsBm().executeTaskSendingPluginMessage(() -> {
											network.server1.getTestsBm().bm.sendPluginNotificationToAll(notif);
										});
									});
								});
							});

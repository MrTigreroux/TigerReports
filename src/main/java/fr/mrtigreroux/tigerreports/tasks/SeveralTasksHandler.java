...
public class SeveralTasksHandler<R> {
     ...
     * @return
     * 
     * @throws IllegalStateException if {@link #whenAllTasksDone(ResultCallback)} has been called
     *                               before
     */
    public ResultCallback<R> newTaskResultSlot() throws IllegalStateException {
        if (this.allTasksAdded) {
            throw new IllegalStateException(
                    "Cannot add a task result slot when all tasks are marked as added"
            );
        }
    ...
    public void whenAllTasksDone(boolean removeNullResults,
            ResultCallback<List<R>> resultCallback) {
    ...
    private void checkAllTasksDone() {
        ...
        synchronized (tasksResult) {
            ...
            if (removeNullResults) {
                ...
                LOGGER.info(
                        () -> this + ": all tasks done, removed " + fremovedAmount + " null results"
                );
            }
        }

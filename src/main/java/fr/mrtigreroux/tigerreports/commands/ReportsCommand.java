...
public class ReportsCommand implements TabExecutor {
    ...
    public boolean onCommand(CommandSender s, Command cmd, String label, String[] args) {
        ...
        if (args.length >= 2 && args[0].equalsIgnoreCase("logs") && Permission.STAFF.check(s)) {
            if (args.length == 2) {
                ...
                String bukkitLoggersShowName = Logger.LOGS_CONFIG_FILE_GLOBAL_SETTING_TRUE_VALUE
                        .equals(args[1]) ? Logger.LOGS_CONFIG_FILE_GLOBAL_SETTING_TRUE_VALUE : "0";
                String bukkitLoggersUseColors = Logger.LOGS_CONFIG_FILE_GLOBAL_SETTING_TRUE_VALUE
                        .equals(args[2]) ? Logger.LOGS_CONFIG_FILE_GLOBAL_SETTING_TRUE_VALUE : "0";
                newConfigLines.add(
        ...
        switch (args.length) {
                    ...
                    case "notify":
                        ...
                        p.sendMessage(
                                Message.STAFF_NOTIFICATIONS.get()
                                        .replace(
                                                "_State_",
                                                (newState ? Message.ACTIVATED : Message.DISABLED)
                                                        .get()
                                        )
                        );
                        return true;
            ...
            case 2:
                switch (args[0].toLowerCase()) {
                    case "deleteall":
                        ...
                        boolean unarchived =
                                reportsType != null && reportsType.equalsIgnoreCase("unarchived");
                        if (
                            unarchived
                                    || (reportsType != null
                                            && reportsType.equalsIgnoreCase("archived"))
                        ) {
            ...
            case 3:
                if (args[0].equalsIgnoreCase("punish")) {
                    ...
                    MessageUtils.sendErrorMessage(
                            s,
                            Message.INVALID_TIME.get().replace("_Time_", args[2])
                    );
                    return true;
                } else {
                    break;
    ...
    private void processCommandWithTarget(User u, String target, String command,
            long punishSeconds) {

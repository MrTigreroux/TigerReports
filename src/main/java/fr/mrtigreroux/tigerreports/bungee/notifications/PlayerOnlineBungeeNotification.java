...
public class PlayerOnlineBungeeNotification extends BungeeNotification {
    ...
    public PlayerOnlineBungeeNotification(long creationTime, String playerName, UUID playerUUID, boolean online,
            String serverName, String lastMessagesStartDatetime) {
        this(creationTime, playerName, playerUUID.toString(), online, serverName, lastMessagesStartDatetime);
    }

    public PlayerOnlineBungeeNotification(long creationTime, String playerName, String playerUniqueId, boolean online,
            String serverName, String lastMessagesStartDatetime) {
        super(creationTime);
    ...
    public void onReceive(Database db, TaskScheduler ts, UsersManager um, ReportsManager rm, VaultManager vm,
        ...
        if (isNotifiable(bm) && Bukkit.getPlayer(playerName) == null) { // prevents changing local online status of an online player on the current server
            ...
            if (online && bm.isValidDifferentServerName(serverName)) {
                ...
                if (cachedUser != null) {
                    List<SavedMessage> lastMessages = cachedUser
                            .getLastMessagesAfterDatetime(lastMessagesStartDatetime);
                    if (CheckUtils.isNotEmpty(lastMessages)) {
                        bm.sendPluginNotificationToServer(serverName, new PlayerLastMessagesBungeeNotification(
                                bm.getNetworkCurrentTime(), playerUUID, lastMessages));
                    }
                }
            }

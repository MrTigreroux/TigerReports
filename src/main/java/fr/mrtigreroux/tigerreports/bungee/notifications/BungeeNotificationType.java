...
public class BungeeNotificationType {
    ...
    private static final Map<Class<? extends BungeeNotification>, BungeeNotificationType> notificationTypeByDataClass =
            new HashMap<>(); // BungeeNotificationType specifically depends on the data class instance and not only on the data class name
    
    static {
        try {
            creationTimeField = Objects
                    .requireNonNull(BungeeNotification.class.getDeclaredField("creationTime"));
        ...
        notificationTypeById =
                notifTypeById.toArray(new BungeeNotificationType[notifTypeById.size()]);
    }
    ...
    public BungeeNotificationType(byte id, Class<? extends BungeeNotification> dataClass) {
        ...
        for (Field field : rawDataFields) {
            ...
            if (fieldType == null) {
                throw new IllegalStateException(
                        "The notification data field " + field.getName() + " of "
                                + dataClass.getSimpleName() + " has an unknown (null) type"
                );
            }
            try {
                dataFields[i] =
                        new NotificationDataField(field, FieldSerializer.getByClass(fieldType));
            } catch (IllegalArgumentException ex) {
                throw new IllegalStateException(
                        "The notification data field " + field.getName() + " of "
                                + dataClass.getSimpleName() + " has an invalid type: "
        ...
        try {
            dataClassConstructor = Arrays.stream(dataClass.getDeclaredConstructors())
                    .filter((constructor) -> constructorSimpleFieldsType.equals(Sets.newHashSet(constructor.getParameterTypes()))).findFirst().get();
        } catch (SecurityException | NullPointerException | NoSuchElementException ex) {
            throw new IllegalStateException(
                    "The constructor of the " + dataClass.getSimpleName()
        ...
        if (dataClassConstructor.getParameterCount() != dataFields.length + 1) {
            throw new IllegalStateException(
                    "The " + dataClass.getSimpleName() + " class has " + dataFields.length
                            + " data fields but its constructor has "
                            + dataClassConstructor.getParameterCount()
    ...
    public static class NotificationDataField {
        ...
        void write(BungeeNotification notif, DataOutputStream outputStream)
    ...
    public static abstract class FieldSerializer {
        ...
        public static final Map<String, FieldSerializer> fieldSerializerByClassName =
                new HashMap<>();
            ...
            if (fieldSerializer == null) {
                if (Boolean.TYPE.getName().equals(className)) {
                    fieldSerializer = new FieldSerializer() {
                        ...
                        @Override
                        void write(Field field, BungeeNotification notif,
                    ...
                    throw new IllegalArgumentException(
                            "Missing field serializer for class " + className
                    );
                }
        ...
        private static void writeString(String toWrite, DataOutputStream outputStream)
                throws IOException {
            if (toWrite == null) {
                toWrite = NULL_VALUE;
            } else if (NULL_VALUE.equals(toWrite)) {
                throw new IllegalArgumentException(
                        "Cannot write the null value as a normal string value"
                );
            }
        ...
        /**
         * This value is used to create an empty notification, but is actually never kept. Because
         * some notification types have some constraints (not null, not empty, not negative...) that
         * are checked in the constructor, the undefined value should not have a simple value like 0
         * or null. If needed, this method could be changed to return a set of different undefined
         * values that could be tested until one works to create a certain type of notifications.
         * 
         * <h5>Why this method exists:</h5> Since it is not possible to know what constructor
         * parameter is associated to a certain field (there is no rule, the parameter can be named
         * differently, parameters can be declared in a different order than the fields declaration
         * order, the parameter can be induced by another parameter...), it is preferable to
         * associate fields value without using the constructor. This method defines the value to
         * use (for a certain field type) in the notification constructor to create a notification
         * instance, before filling its fields with their real value. By defining this method in
         * this class, each field type is guaranteed to have an undefined value, and no oversight is
         * possible when adding the support of a new field type.
         */
    ...
    public BungeeNotification newEmptyNotification() {
        ...
        try {
            notif = (BungeeNotification) dataClassConstructor
                    .newInstance(dataClassConstructorParametersDefaultValue);
        } catch (

...
public class BungeeNotificationType {
	...
	private static void addBungeeNotificationType(Class<? extends BungeeNotification> dataClass,
		...
		if (notificationTypeByDataClass.containsKey(dataClass)) {
			throw new IllegalStateException("The notification data class " + dataClass + " has already been registered");
		}
	...
	public BungeeNotificationType(byte id, Class<? extends BungeeNotification> dataClass) {
		...
		try {
			dataClassConstructor = Arrays.stream(dataClass.getDeclaredConstructors()).filter((constructor) -> constructorSimpleFieldsType.equals(Sets.newHashSet(constructor.getParameterTypes()))).findFirst().get();
			...
			if (fieldSerializer == null) {
				if (Boolean.TYPE.getName().equals(className)) {
					...
					fieldSerializer = new FieldSerializer() {
						...
						@Override
						void write(Field field, BungeeNotification notif, DataOutputStream outputStream) throws IOException, IllegalAccessException {

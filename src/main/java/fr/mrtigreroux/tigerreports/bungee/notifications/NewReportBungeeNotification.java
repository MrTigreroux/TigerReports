...
public class NewReportBungeeNotification extends BungeeNotification {
    ...
    public void onReceive(Database db, TaskScheduler ts, UsersManager um, ReportsManager rm, VaultManager vm,
        ...
        if (reportData == null) {
            LOGGER.info(() -> "processNewReportMessage(): reportData = null, reportDataAsString = " + reportBasicData);
            return;
        }
        ...
        LOGGER.info(() -> "processNewReportMessage(): reportData = " + CollectionUtils.toString(reportData)
                + ", isRecent = " + isRecent + ", notify = " + notify);

        if (isRecent) {
            rm.updateAndGetReport(reportId, reportData, false, false, db, ts, um, createNewReportResultCallback(notify,
                    reportServer, missingData, reportBasicData, reportData, db, ts, um, vm, bm));
        } else if (notify) {
            rm.getReportByIdAsynchronously(reportId, false, false, db, ts, um, createNewReportResultCallback(true,
                    reportServer, missingData, reportBasicData, reportData, db, ts, um, vm, bm));
        } else {
    ...
    private ResultCallback<Report> createNewReportResultCallback(boolean notify, String reportServer,
            boolean reportMissingData, String reportDataAsString, Map<String, Object> reportData, Database db,
            TaskScheduler ts, UsersManager um, VaultManager vm, BungeeManager bm) {
        return (r) -> {
            if (r != null && r.getBasicDataAsString().equals(reportDataAsString)) {
                sendReportAndImplementMissingData(r, reportServer, notify, reportMissingData, db, vm, bm);
            } else {
                sendReportWithReportData(reportData, reportServer, notify, reportMissingData, db, ts, um, vm, bm);
            }
        };
    }
    ...
    private void sendReportWithReportData(Map<String, Object> reportData, String reportServer, boolean notify,
            boolean reportMissingData, Database db, TaskScheduler ts, UsersManager um, VaultManager vm,
            BungeeManager bm) {
    ...
    public String toString() {
        return "NewReportBungeeNotification [reportServer=" + reportServer + ", missingData=" + missingData
                + ", reportBasicData=" + reportBasicData + "]";
    }

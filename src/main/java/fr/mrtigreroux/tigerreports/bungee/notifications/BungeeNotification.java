...
public abstract class BungeeNotification {
    ...
    /**
     * NB: The notification creation time can be really different of its sending time to a certain
     * server.
     */
    ...
    public abstract void onReceive(Database db, TaskScheduler ts, UsersManager um,
    ...
    protected void getReportAsync(int reportId, Database db, TaskScheduler ts, UsersManager um,
            ReportsManager rm, BungeeManager bm, Logger logger,
            ResultCallback<Report> resultCallback) {
        rm.getReportByIdAsynchronously(reportId, false, isNotifiable(bm), db, ts, um, (r) -> {
            if (r == null) {
                logger.log(
                        isNotifiable(bm) ? Level.WARN : Level.INFO,
                        () -> "Report #" + reportId
    ...
    protected void getUserAsyncIfNotifiable(String userUniqueId, Database db, TaskScheduler ts,
            UsersManager um, BungeeManager bm, ResultCallback<User> resultCallback) {
        if (isNotifiable(bm)) {

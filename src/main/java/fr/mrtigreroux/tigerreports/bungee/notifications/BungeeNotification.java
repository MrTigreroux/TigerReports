...
public abstract class BungeeNotification {
	...
	protected void getReportAsync(int reportId, Database db, TaskScheduler ts, UsersManager um, ReportsManager rm, BungeeManager bm, Logger logger, ResultCallback<Report> resultCallback) {
		rm.getReportByIdAsynchronously(reportId, false, isNotifiable(bm), db, ts, um, (r) -> {
			if (r == null) {
				logger.log(isNotifiable(bm) ? Level.WARN : Level.INFO, () -> "Report #" + reportId + " is null (has probably been deleted since), ignoring notification");
				return;
			}
	...
	protected void getUserAsyncIfNotifiable(String userUniqueId, Database db, TaskScheduler ts, UsersManager um, BungeeManager bm, ResultCallback<User> resultCallback) {
		if (isNotifiable(bm)) {

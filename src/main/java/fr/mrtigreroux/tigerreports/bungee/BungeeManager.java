...
public class BungeeManager implements PluginMessageListener {
	...
	private static class KnownServer {
		...
		private List<BungeeNotification> unsentNonEphemeralNotifications = new CollectionUtils.LimitedOrderedList<>(
		        MAX_OFFLINE_SERVER_KEPT_UNSENT_NON_EPHEMERAL_NOTIFICATIONS);
		private List<BungeeNotification> unsentEphemeralNotifications = new CollectionUtils.LimitedOrderedList<>(
		...
		public void addUnsentNotification(boolean ephemeralNotification, BungeeNotification notification) {
		...
		public List<BungeeNotification> getAndClearUnsentNotifications(boolean ephemeralNotifications) {
			return getAndClearUnsentNotifications(
			        ephemeralNotifications ? unsentEphemeralNotifications : unsentNonEphemeralNotifications);
		}

		private List<BungeeNotification> getAndClearUnsentNotifications(List<BungeeNotification> unsentNotifications) {
	...
	private List<BungeeNotification> unsentNotificationsWhenServersUnknown = new CollectionUtils.LimitedOrderedList<>(
	...
	private void addPlayerOnlineCallback(String playerName, ResultCallback<Player> onlineCallback) {
		CollectionUtils.LimitedOrderedList<ResultCallback<Player>> playerOnlineCallbacks = onlineCallbacksByPlayerName.get(playerName);
		if (playerOnlineCallbacks == null) {
			playerOnlineCallbacks = new CollectionUtils.LimitedOrderedList<>(MAX_ONLINE_CALLBACKS_BY_PLAYER);
	...
	public void processPlayerConnection(Player p) {
		...
		sendPluginNotificationToAll(new PlayerOnlineBungeeNotification(getNetworkCurrentTime(), playerOfflineNotBroadcastedName, curPlayerOfflineNotBroadcastedUUID, false, localServerName, null));
		});
		...
		if (onlineUser != null) {
			updatePlayerOnlineInNetwork(p.getName(), true, p.getUniqueId(), onlineUser.getLastMessagesMinDatetimeOfInsertableMessages());
	...
	private void processPlayerOnlineCallbacks(Player p) {
		...
		LOGGER.debug(() -> "processPlayerOnlineCallbacks(" + playerName + "): callbacks = " + CollectionUtils.toString(playerOnlineCallbacks));
	...
	public void sendPluginNotificationToAll(BungeeNotification notif) {
		...
		if (knownServers.isEmpty()) {
			...
			LOGGER.info(() -> "sendPluginNotificationToAll(): servers are unknown, saved notification " + notif
			        + " to try to send it later when servers will be collected");
		} else {
			LOGGER.info(() -> "sendPluginNotificationToAll(): attempt to send " + notif + " to all known servers...");
	...
	private boolean sendPluginNotificationTo(String networkTarget, BungeeNotification notif) {
		...
		try {
			...
			Logger.BUNGEE.info(() -> "<-- SENT (to: " + networkTarget + "): " + notifType.toString(notif));
			return true;
		} catch (Exception ex) {
	...
	public void onPluginMessageReceived(String channel, Player player, byte[] messageReceived) {
		...
		try {
			...
			case "TigerReports":
				...
				DataInputStream msgDataIn = new DataInputStream(new ByteArrayInputStream(msgBytes));

				BungeeNotificationType notifType = BungeeNotificationType.getById(msgDataIn.readByte());
				BungeeNotification notif = notifType.readNotification(msgDataIn);
				Logger.BUNGEE.info(() -> "--> RECEIVED (sent at: " + notif.creationTime + ", elapsed: "
				        + notif.getElapsedTime(BungeeManager.this) + "ms): " + notifType.toString(notif));
	...
	private void sendAllUnsentNotificationsOfServer(KnownServer server) {
		if (server.isOnline()) {
			List<BungeeNotification> unsentEphemeralNotifications = server.getAndClearUnsentNotifications(true);
			...
			List<BungeeNotification> unsentNonEphemeralNotifications = server.getAndClearUnsentNotifications(false);
	...
	private void updatePlayerOnlineInNetwork(String playerName, boolean online, UUID playerUUID,
		...
		if (Bukkit.getOnlinePlayers().size() >= (online ? 1 : 2)) { // if the notif is for an offline player, that offline player can still be counted in Bukkit.getOnlinePlayers() for short time
			getServerName((localServerName) -> {
			sendPluginNotificationToAll(new PlayerOnlineBungeeNotification(getNetworkCurrentTime(), playerName, playerUUID, online, localServerName, lastMessagesStartDatetime));
			});
		} else {
			LOGGER.info(
	...
	public void tpPlayerToOtherServerLocation(String playerName, String serverName, String serializedLoc) {
		KnownServer server = getKnownServer(serverName);
		if (server == null) {
		...
		sendPluginNotificationToServer(server, new TeleportToLocationBungeeNotification(getNetworkCurrentTime(), playerName, serializedLoc));
	...
	public void tpPlayerToPlayerInOtherServer(String playerName, String targetName) {
		sendPluginNotificationToAll(new TeleportToPlayerBungeeNotification(getNetworkCurrentTime(), playerName, targetName));
	}

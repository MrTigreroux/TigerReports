...
public class User {
        ...
        if (!bungee) {
            if (bm != null) {
                bm.sendPluginNotificationToAll(
                        new ImmunityBungeeNotification(bm.getNetworkCurrentTime(), uuid, this.immunity));
            }
    ...
    public void punish(long seconds, User staff, boolean bungee, Database db, BungeeManager bm, VaultManager vm) {
        ...
        if (!bungee) {
            ...
            if (staff != null) {
                bm.sendPluginNotificationToAll(
                        new PunishBungeeNotification(bm.getNetworkCurrentTime(), staff.getUniqueId(), uuid, seconds));
            }
        }
        if (staff != null) {
    ...
    public void stopCooldown(User staff, boolean bungee, Database db, BungeeManager bm) {
        ...
        if (staff != null) {
            ...
            if (!bungee) {
                bm.sendPluginNotificationToAll(
                        new StopCooldownBungeeNotification(bm.getNetworkCurrentTime(), staff.getUniqueId(), uuid));
            }
        }
    }
    ...
    public void changeStatistic(String statisticName, int relativeValue, boolean bungee, Database db,
        ...
        if (!bungee) {
            if (bm != null) {
                bm.sendPluginNotificationToAll(new StatisticBungeeNotification(bm.getNetworkCurrentTime(), uuid,
                        statisticName, relativeValue));
            }
    ...
    public void toggleCommentSentState(Comment c, Database db, TaskScheduler taskScheduler, VaultManager vm,
        ...
        if (isPrivate && isOnline()) {
            ...
            if (isPrivate && bungeeServerName != null) {
                bm.sendPluginNotificationToServer(bungeeServerName,
                        new CommentBungeeNotification(bm.getNetworkCurrentTime(), reportId, commentId, getUniqueId()));
            } else {

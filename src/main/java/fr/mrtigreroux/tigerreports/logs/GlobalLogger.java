...
public enum GlobalLogger {
    ...
    public Level getConfiguredLoggerLevel(List<String> configLines, String pluginName) {
        ...
        try {
            return Objects.requireNonNull(
                    Level.fromDisplayName(configLines.get(getConfigLine()).substring(getLoggerName().length() + Logger.LOGS_CONFIG_FILE_GLOBAL_LOGGER_DATA_SEPARATOR.length()))
            );
        } catch (IndexOutOfBoundsException | NullPointerException ex) {
            Bukkit.getLogger()
                    .log(
                            java.util.logging.Level.SEVERE,
                            "[" + pluginName + "] Invalid " + Logger.LOGS_CONFIG_FILE_NAME
                                    + " file for " + getLoggerName() + ": "
    ...
    public BukkitLogger createBukkitLogger(List<String> configLines, String pluginName,
            boolean showName, boolean useColors) {
        return new BukkitLogger(
                getLoggerName(),
                pluginName,
                getConfiguredLoggerLevel(configLines, pluginName),
                showName,
                useColors
        );
    }

...
public abstract class Logger {
    ...
    static {
        ...
        if (USE_BUKKIT_LOGGER) {
            ...
            try {
                configLines = FileUtils
                        .getFileLines(FileUtils.getPluginDataFile(plugin, LOGS_CONFIG_FILE_NAME));
                
                if (!configLines.isEmpty() && configLines.size() >= 2) {
                    String[] firstLineParams =
                            configLines.get(0).split(LOGS_CONFIG_FILE_GLOBAL_SETTINGS_SEPARATOR);
                    bukkitLoggersShowName =
                            LOGS_CONFIG_FILE_GLOBAL_SETTING_TRUE_VALUE.equals(firstLineParams[0]);
                    bukkitLoggersUseColors =
                            LOGS_CONFIG_FILE_GLOBAL_SETTING_TRUE_VALUE.equals(firstLineParams[1]);
                    
                    setDefaultClassLoggerLevel(
                            GlobalLogger.CLASS.getConfiguredLoggerLevel(configLines, pluginName)
                    );
                } else {
                    Bukkit.getLogger()
                            .log(
                                    java.util.logging.Level.SEVERE,
                                    "[" + pluginName + "] Invalid " + LOGS_CONFIG_FILE_NAME
            ...
            MAIN = GlobalLogger.MAIN.createBukkitLogger(
                    configLines,
                    pluginName,
                    bukkitLoggersShowName,
                    bukkitLoggersUseColors
            );
            SQL = GlobalLogger.SQL.createBukkitLogger(
                    configLines,
                    pluginName,
                    bukkitLoggersShowName,
                    bukkitLoggersUseColors
            );
            BUNGEE = GlobalLogger.BUNGEE.createBukkitLogger(
                    configLines,
                    pluginName,
                    bukkitLoggersShowName,
                    bukkitLoggersUseColors
            );
            EVENTS = GlobalLogger.EVENTS.createBukkitLogger(
                    configLines,
                    pluginName,
                    bukkitLoggersShowName,
                    bukkitLoggersUseColors
            );
            CONFIG = GlobalLogger.CONFIG.createBukkitLogger(
                    configLines,
                    pluginName,
                    bukkitLoggersShowName,
                    bukkitLoggersUseColors

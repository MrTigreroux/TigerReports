...
public class ArchivedReportsMenu extends Menu implements UpdatedMenu {
	...
	public void onUpdate(Inventory inv) {
		ReportUtils.addReports(true, inv, page, Message.REPORT_RESTORE_ACTION.get()+(Permission.STAFF_DELETE.isOwned(u) ? Message.REPORT_DELETE_ACTION
				.get() : ""));
	}
	...
	public void onClick(ItemStack item, int slot, ClickType click) {
		if (slot == 0) {
			...
			Report r = TigerReports.getInstance()
					.getReportsManager()
					.formatFullReport(TigerReports.getInstance()
							.getDb()
							.query("SELECT * FROM tigerreports_reports WHERE archived = ? LIMIT 1 OFFSET ?", Arrays.asList(1, getConfigIndex(slot)-1))
							.getResult(0));
			if (r == null) {
				update(true);

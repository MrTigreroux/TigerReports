...
public class ReportMenu extends ReportManagerMenu {
	...
	public Inventory onOpen() {
		...
		inv.setItem(22, MenuItem.PUNISH_ABUSE.getWithDetails(Message.PUNISH_ABUSE_DETAILS.get()
		...
		for(String type : new String[]{"Reporter", "Reported"}) {
			...
			Map<String, Object> statistics = statisticsQuery.getResult(type.equals("Reporter") ? 0 : 1);
		...
		for(Status status : Status.values()) {
			inv.setItem(statusPosition, new CustomItem()
					.type(Material.STAINED_CLAY)
					.damage(status.getColor())
					.glow(status.equals(r.getStatus()))
					.name(status == Status.DONE ? Message.PROCESS_STATUS.get() : Message.CHANGE_STATUS.get().replace("_Status_", status.getWord(null)))
					.lore((status == Status.DONE ? Message.PROCESS_STATUS_DETAILS.get() : Message.CHANGE_STATUS_DETAILS.get()).replace("_Status_", status.getWord(null)).split(ConfigUtils.getLineBreakSymbol()))
					.create());
	...
	public void onClick(ItemStack item, int slot, ClickType click) {
		switch(slot) {
			case 0:
				...
				break;
			case 18:
				u.printInChat(r, r.implementDetails(Message.REPORT_CHAT_DETAILS.get(), false).replace("_Report_", r.getName()).split(ConfigUtils.getLineBreakSymbol()));
				break;
			case 21: case 23:
			...
			case 26:
				if(click == ClickType.LEFT) {
					...
					for(String type : new String[] {"Reported", "Reporter"}) {
						for(String message : r.getMessagesHistory(type)) {
							if(message != null && message.length() >= 20) {
								...
								sortedMessages.put(MessageUtils.getSeconds(date), Message.REPORT_MESSAGE_FORMAT.get()
										.replace("_Date_", date)
					...
					u.printInChat(r, Message.REPORT_MESSAGES_HISTORY.get()
							.replace("_Report_", r.getName())
							.replace("_Messages_", !messages.toString().isEmpty() ? messages.toString() : Message.NONE_MALE.get())
							.split(ConfigUtils.getLineBreakSymbol()));
				}
				break;
			case 36:

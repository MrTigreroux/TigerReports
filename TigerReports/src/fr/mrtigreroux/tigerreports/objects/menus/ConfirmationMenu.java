...
public class ConfirmationMenu extends ReportManagerMenu {
	...
	public Inventory onOpen() {
		...
		inv.setItem(11, new CustomItem()
				.type(Material.STAINED_CLAY)
				.damage((byte) 5)
				.name(Message.valueOf("CONFIRM_"+actionDisplayed).get())
				.lore(Message.valueOf("CONFIRM_"+actionDisplayed+"_DETAILS").get().replace("_Report_", report).split(ConfigUtils.getLineBreakSymbol()))
				.create());
		inv.setItem(13, r.getItem(null));
		inv.setItem(15, new CustomItem()
				.type(Material.STAINED_CLAY)
				.damage((byte) 14)

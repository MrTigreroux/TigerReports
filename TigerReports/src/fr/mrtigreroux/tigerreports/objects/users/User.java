...
public abstract class User {
	...
	public void startImmunity(boolean bungee) {
		updateImmunity(MessageUtils.convertToDate(MessageUtils.getSeconds(MessageUtils.getNowDate())
		        + (ConfigFile.CONFIG.get().getInt("Config.ReportedImmunity", 120))), bungee);
	}
	...
	public String getImmunity() {
		...
		if (immunity == null) {
			immunity = (String) TigerReports.getInstance().getDb()
	...
	public void updateCooldown(String cooldown, boolean bungee) {
		...
		if (!bungee) {
			...
			tr.getDb().updateAsynchronously("UPDATE tigerreports_users SET cooldown = ? WHERE uuid = ?",
			        Arrays.asList(cooldown, uuid));
		}
	}

	public void startCooldown(double seconds, boolean bungee) {
		updateCooldown(MessageUtils.convertToDate(MessageUtils.getSeconds(MessageUtils.getNowDate()) + seconds),
		        bungee);
	}

	public void punish(double seconds, String staffUuid, boolean bungee) {
		String time = MessageUtils.convertToSentence(seconds);
		if (staffUuid != null) {
			MessageUtils.sendStaffMessage(
			        Message.STAFF_PUNISH.get().replace("_Player_", UserUtils.getDisplayName(staffUuid, true))
			                .replace("_Reporter_", getDisplayName(false)).replace("_Time_", time),
			        ConfigSound.STAFF.get());
			sendMessage(Message.PUNISHED.get().replace("_Time_", time));
		}
		if (!bungee)
			TigerReports.getInstance().getBungeeManager()
	...
	public String getCooldown() {
		if (cooldown == null || (cooldown.startsWith("|")
			...
			cooldown = (String) TigerReports.getInstance().getDb()
	...
	public void stopCooldown(String staffUuid, boolean bungee) {
		if (staffUuid != null) {
			MessageUtils.sendStaffMessage(
			        Message.STAFF_STOPCOOLDOWN.get().replace("_Player_", UserUtils.getDisplayName(staffUuid, true))
	...
	public Map<String, Integer> getStatistics() {
		...
		Map<String, Object> result = TigerReports.getInstance().getDb().query(
	...
	public void setStatistic(String statistic, int value, boolean bungee) {
		...
		if (!bungee) {
			...
			tr.getDb().updateAsynchronously("UPDATE tigerreports_users SET " + statistic + " = ? WHERE uuid = ?",
	...
	public List<String> getNotifications() {
		...
		try {
			notifications = new ArrayList<>(
			        Arrays.asList(
			                ((String) TigerReports.getInstance().getDb()
	...
	public void setNotifications(List<String> notifications) {
		TigerReports.getInstance().getDb().update("UPDATE tigerreports_users SET notifications = ? WHERE uuid = ?",

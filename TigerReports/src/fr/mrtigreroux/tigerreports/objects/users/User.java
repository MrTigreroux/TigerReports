...
public abstract class User {
	...
	public void punish(double seconds, String staff, boolean bungee) {
		...
		if (staff != null) {
			MessageUtils.sendStaffMessage(Message.STAFF_PUNISH.get().replace("_Player_", staff)
					.replace("_Reporter_", getName()).replace("_Time_", time), ConfigSound.STAFF.get());
			sendMessage(Message.PUNISHED.get().replace("_Time_", time));
		}
		if (!bungee)
			TigerReports.getInstance().getBungeeManager()
					.sendPluginNotification(staff + " punish user " + uuid + " " + seconds);
	...
	public void stopCooldown(String staff, boolean bungee) {
		if (staff != null) {
			MessageUtils.sendStaffMessage(
					Message.STAFF_STOPCOOLDOWN.get().replace("_Player_", staff).replace("_Target_", getName()),

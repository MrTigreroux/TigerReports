...
public class ReportsNotifier implements Runnable {
	...
	public static String getReportsNotification() {
		...
		for (Map<String, Object> result : TigerReports.getInstance()
				.getDb()
				.query("SELECT status FROM tigerreports_reports WHERE archived = ?", Collections.singletonList(0))
				.getResultList()) {
		...
		for (Status status : Status.values()) {
			...
			reportsNotification = reportsNotification.replace(statusPlaceHolder, (amount <= 1 ? Message.REPORT_TYPE : Message.REPORTS_TYPE).get()

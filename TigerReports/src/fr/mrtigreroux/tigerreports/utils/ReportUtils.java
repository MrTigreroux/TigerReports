...
public class ReportUtils {
	...
	public static ItemStack getItem(int reportNumber, String actions) {
		...
		return new CustomItem().type(status.getMaterial()).hideFlags(true).glow(status.equals(Status.WAITING)).name(Message.REPORT.get().replaceAll("_Report_", getName(reportNumber)))
				.lore(Message.REPORT_DETAILS.get().replaceAll("_Status_", status.getWord()+(status.equals(Status.DONE) ? Message.APPRECIATION_SUFFIX.get().replaceAll("_Appreciation_", getAppreciation(reportNumber)) : "")).replaceAll("_Appreciation_", getAppreciation(reportNumber)).replaceAll("_Date_", getDate(reportNumber))
						.replaceAll("_Signalman_", getPlayerName("Signalman", reportNumber, true)).replaceAll("_Reported_", getPlayerName("Reported", reportNumber, true))
